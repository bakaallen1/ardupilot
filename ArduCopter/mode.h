...
public:
    ...
    enum class Number : uint8_t {
        ...
        DRAWSTAR =     29,  // Draw a star based on current position
        DRAWSQURE =    30,
    };
...
public:
    ...
    SubMode submode() const { return drawstar_mode; }
...
private:
    ...
    // controls which controller is run (pos or vel): WP only
    SubMode drawstar_mode = SubMode::WP;
    bool send_notification;     // used to send one time notification to ground station
    bool takeoff_complete;      // true once takeoff has completed (used to trigger retracting of landing gear)
...
class ModeDrawSqure: public Mode {

public:
    // inherit constructor
    using Mode::Mode;
    Number mode_number() const override { return Number::DRAWSQURE; }
    ...
    SubMode submode() const { return drawsqure_mode; }
...
protected:
    ...
    const char *name() const override { return "DRAWSQURE"; }
    const char *name4() const override { return "DRAW"; }
...
private:
    ...
    // controls which controller is run (pos or vel): WP only
    SubMode drawsqure_mode = SubMode::WP;
    bool send_notification;     // used to send one time notification to ground station
    bool takeoff_complete;      // true once takeoff has completed (used to trigger retracting of landing gear)
