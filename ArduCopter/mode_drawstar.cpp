...
static Vector3p drawstar_pos_target_cm;       // position target (used by posvel controller only)
bool drawstar_pos_terrain_alt;                // true if drawstar_pos_target_cm.z is an alt above terrain
static Vector3f drawstar_vel_target_cms;      // velocity target (used by pos_vel_accel controller and vel_accel controller)
static Vector3f drawstar_accel_target_cmss;   // acceleration target (used by pos_vel_accel controller vel_accel controller and accel controller)
...
struct {
    ...
    bool use_yaw_rate;
    bool use_thrust;
} static drawstar_angle_state;

struct Drawstar_Limit {
    uint32_t timeout_ms;  // timeout (in seconds) from the time that guided(drawstar) is invoked
    float alt_min_cm;   // lower altitude limit in cm above home (0 = no limit)
    float alt_max_cm;   // upper altitude limit in cm above home (0 = no limit)
    float horiz_max_cm; // horizontal position limit in cm from where guided(drawstar) mode was initiated (0 = no limit)
    uint32_t start_time;// system time in milliseconds that control was handed to the external computer
    Vector3f start_pos; // start position as a distance from home in cm.  used for checking horiz_max limit
} drawstar_limit;

// init - initialise drawstar controller
bool ModeDrawStar::init(bool ignore_checks)
{
    //// start in velaccel control mode
    //velaccel_control_start();
    // generate_path
    gcs().send_text(MAV_SEVERITY_INFO, "Init DrawStar mode");
    path_num = 0;
    // start in pos control mode
   //wp_control_start();
    drawstar_vel_target_cms.zero();
    drawstar_accel_target_cmss.zero();
    send_notification = false;
    
    // clear pause state when entering guided mode
    _paused = false;
    do_user_takeoff_start(2000.0f);
        
    gcs().send_text(MAV_SEVERITY_INFO, "DrawStar mode initialized successfully");
    return true;
}
// wp calculator
...
void ModeDrawStar::run()
{
    ...
    if (_paused) {
        gcs().send_text(MAV_SEVERITY_INFO, "DrawStar mode paused");
        pause_control_run();
        return;
    }

    // call the correct auto controller
    switch (drawstar_mode) {
    ...
    case SubMode::TakeOff:
        ...
        if (takeoff_complete) {
            ...
            generate_path();
            wp_control_start();
            gcs().send_text(MAV_SEVERITY_INFO, "Takeoff completed. Switching to SubMode: WP");
        }
        if (now - last_debug_time_ms > 1000) {
            gcs().send_text(MAV_SEVERITY_INFO, "Current DrawStar SubMode: TakeOff");
    ...
    case SubMode::WP:
        ...
        if(path_num<5){
            ...
            if(wp_nav->reached_wp_destination()){
                ...
                gcs().send_text(MAV_SEVERITY_INFO, "Current DrawStar SubMode: WP");
            }
          
        }
        else if (path_num == 5 && wp_nav->reached_wp_destination()) {
                // Once the last waypoint is reached, send a message and switch to Land mode
            gcs().send_text(MAV_SEVERITY_INFO, "Draw star finished, now transitioning to Land Mode.");
    ...
    case SubMode::Pos:
        ...
        pos_control_run();
        gcs().send_text(MAV_SEVERITY_INFO, "Current DrawStar SubMode: Pos");
        break;

    case SubMode::Accel:
        accel_control_run();
        gcs().send_text(MAV_SEVERITY_INFO, "Current DrawStar SubMode: Accel");
        break;

    case SubMode::VelAccel:
        velaccel_control_run();
        gcs().send_text(MAV_SEVERITY_INFO, "Current DrawStar SubMode: VelAccel");
        break;

    case SubMode::PosVelAccel:
        posvelaccel_control_run();
        gcs().send_text(MAV_SEVERITY_INFO, "PosVelAccel");
        break;

    case SubMode::Angle:
        angle_control_run();
        gcs().send_text(MAV_SEVERITY_INFO, "Current DrawStar SubMode: Angle");
        break;
    }
 }
...
{
    copter.gcs().send_text(MAV_SEVERITY_INFO, "Checking if arming is allowed");
    // always allow arming from the ground station
    if (method == AP_Arming::Method::MAVLINK) {
        copter.gcs().send_text(MAV_SEVERITY_INFO, "Arming allowed via MAVLINK");
        return true;
    }
    ...
    bool ModeDrawStar::allows_arming(AP_Arming::Method method) const
{
    ...
    // optionally allow arming from the transmitter
    bool allowed = (copter.g2.guided_options & (uint32_t)Options::AllowArmingFromTX) != 0;
    copter.gcs().send_text(MAV_SEVERITY_INFO, allowed ? "Arming allowed from transmitter" : "Arming not allowed from transmitter");
    return allowed;
};
...
{
    gcs().send_text(MAV_SEVERITY_INFO, "Starting takeoff in DrawStar mode");
    drawstar_mode = SubMode::TakeOff;
    ...
    if (wp_nav->rangefinder_used_and_healthy() &&
        ...
        if (takeoff_alt_cm <= copter.rangefinder_state.alt_cm) {
            gcs().send_text(MAV_SEVERITY_INFO, "Takeoff failed: target altitude is below current altitude");
            return false;
        }
        ...
        if (!target_loc.get_alt_cm(Location::AltFrame::ABOVE_ORIGIN, alt_target_cm)) {
            gcs().send_text(MAV_SEVERITY_INFO, "Takeoff failed: unable to get target altitude");
    ...
    gcs().send_text(MAV_SEVERITY_INFO, "Takeoff initiated successfully");
    return true;
}
...
void ModeDrawStar::pva_control_start()
{
    ...
    drawstar_pos_terrain_alt = false;
}
//自定修改水平和垂直的速度
/* 
void ModeDrawStar::pva_control_start()
{
    // Set horizontal speed and acceleration to lower values for slower flight
    float max_speed_xy = 50.0; // maximum horizontal speed in cm/s (0.5 m/s)
    float max_accel_xy = 25.0; // maximum horizontal acceleration in cm/s^2 (0.25 m/s^2)

    pos_control->set_max_speed_accel_xy(max_speed_xy, max_accel_xy);
    pos_control->set_correction_speed_accel_xy(max_speed_xy, max_accel_xy);

    // Set vertical speeds and acceleration to lower values for slower ascent/descent
    float max_speed_up = 25.0;   // maximum vertical speed up in cm/s (0.25 m/s)
    float max_speed_down = 15.0; // maximum vertical speed down in cm/s (0.15 m/s)
    float max_accel_z = 15.0;    // maximum vertical acceleration in cm/s^2 (0.15 m/s^2)

    pos_control->set_max_speed_accel_z(max_speed_down, max_speed_up, max_accel_z);
    pos_control->set_correction_speed_accel_z(max_speed_down, max_speed_up, max_accel_z);
...
{
    // set drawstar_mode to velocity controller
    drawstar_mode = SubMode::VelAccel;
...
{
    // set drawstar_mode to velocity controller
    drawstar_mode = SubMode::PosVelAccel;
...
{
    return drawstar_mode == SubMode::TakeOff && !takeoff_complete;
}
...
void ModeDrawStar::angle_control_start()
{
    ...
    // initialise targets
    drawstar_angle_state.update_time_ms = millis();
    drawstar_angle_state.attitude_quat.initialise();
    drawstar_angle_state.ang_vel.zero();
    drawstar_angle_state.climb_rate_cms = 0.0f;
    drawstar_angle_state.yaw_rate_cds = 0.0f;
    drawstar_angle_state.use_yaw_rate = false;
...
bool ModeDrawStar::set_destination(const Vector3f& destination, bool use_yaw, float yaw_cd, bool use_yaw_rate, float yaw_rate_cds, bool relative_yaw, bool terrain_alt)
{
#if AC_FENCE == ENABLED
    ...
    if (use_wpnav_for_position_control()) {
        ...
        // log target
        //copter.Log_Write_Guided_Position_Target(drawstar_mode, destination, terrain_alt, Vector3f(), Vector3f());
    ...
    drawstar_pos_target_cm = destination.topostype();
    drawstar_pos_terrain_alt = terrain_alt;
    drawstar_vel_target_cms.zero();
    drawstar_accel_target_cmss.zero();
    update_time_ms = millis();

    // log target
    //copter.Log_Write_Guided_Position_Target(drawstar_mode, drawstar_pos_target_cm.tofloat(), drawstar_pos_terrain_alt, drawstar_vel_target_cms, drawstar_accel_target_cmss);
...
bool ModeDrawStar::get_wp(Location& destination) const
{
    ...
    case SubMode::Pos:
        destination = Location(drawstar_pos_target_cm.tofloat(), drawstar_pos_terrain_alt ? Location::AltFrame::ABOVE_TERRAIN : Location::AltFrame::ABOVE_ORIGIN);
        return true;
    default:
        return false;
    }
...
bool ModeDrawStar::set_destination(const Location& dest_loc, bool use_yaw, float yaw_cd, bool use_yaw_rate, float yaw_rate_cds, bool relative_yaw)
{
    ...
    if (use_wpnav_for_position_control()) {
        ...
        // log target
        //copter.Log_Write_Guided_Position_Target(drawstar_mode, Vector3f(dest_loc.lat, dest_loc.lng, dest_loc.alt), (dest_loc.get_alt_frame() == Location::AltFrame::ABOVE_TERRAIN), Vector3f(), Vector3f());
        //send_notification = true;
        return true;
    }
    ...
    drawstar_pos_target_cm = pos_target_f.topostype();
    drawstar_pos_terrain_alt = terrain_alt;
    drawstar_vel_target_cms.zero();
    drawstar_accel_target_cmss.zero();
    update_time_ms = millis();
    ...
    // log target
    //copter.Log_Write_Guided_Position_Target(drawstar_mode, Vector3f(dest_loc.lat, dest_loc.lng, dest_loc.alt), drawstar_pos_terrain_alt, drawstar_vel_target_cms, drawstar_accel_target_cmss);
...
void ModeDrawStar::set_accel(const Vector3f& acceleration, bool use_yaw, float yaw_cd, bool use_yaw_rate, float yaw_rate_cds, bool relative_yaw, bool log_request)
{
    // check we are in velocity control mode
    if (drawstar_mode != SubMode::Accel) {
        accel_control_start();
    }
    ...
    drawstar_pos_target_cm.zero();
    drawstar_pos_terrain_alt = false;
    drawstar_vel_target_cms.zero();
    drawstar_accel_target_cmss = acceleration;
    update_time_ms = millis();
...
// set_velaccel - sets guided mode's target velocity and acceleration
void ModeDrawStar::set_velaccel(const Vector3f& velocity, const Vector3f& acceleration, bool use_yaw, float yaw_cd, bool use_yaw_rate, float yaw_rate_cds, bool relative_yaw, bool log_request)
{
    // check we are in velocity control mode
    if (drawstar_mode != SubMode::VelAccel) {
        velaccel_control_start();
    }
    ...
    drawstar_pos_target_cm.zero();
    drawstar_pos_terrain_alt = false;
    drawstar_vel_target_cms = velocity;
    drawstar_accel_target_cmss = acceleration;
    update_time_ms = millis();
...
bool ModeDrawStar::set_destination_posvelaccel(const Vector3f& destination, const Vector3f& velocity, const Vector3f& acceleration, bool use_yaw, float yaw_cd, bool use_yaw_rate, float yaw_rate_cds, bool relative_yaw)
{
#if AC_FENCE == ENABLED
    // reject destination if outside the fence
    ...
    // check we are in velocity control mode
    if (drawstar_mode != SubMode::PosVelAccel) {
...
bool ModeDrawStar::set_attitude_target_provides_thrust() const
{
...
bool ModeDrawStar::stabilizing_pos_xy() const
{
...
bool ModeDrawStar::stabilizing_vel_xy() const
{
...
bool ModeDrawStar::use_wpnav_for_position_control() const
{
...
void ModeDrawStar::set_angle(const Quaternion &attitude_quat, const Vector3f &ang_vel, float climb_rate_cms_or_thrust, bool use_thrust)
{
    // check we are in velocity control mode
    if (drawstar_mode != SubMode::Angle) {
        angle_control_start();
    }

    drawstar_angle_state.attitude_quat = attitude_quat;
    drawstar_angle_state.ang_vel = ang_vel;

    drawstar_angle_state.use_thrust = use_thrust;
    if (use_thrust) {
        drawstar_angle_state.thrust = climb_rate_cms_or_thrust;
        drawstar_angle_state.climb_rate_cms = 0.0f;
    } else {
        drawstar_angle_state.thrust = 0.0f;
        drawstar_angle_state.climb_rate_cms = climb_rate_cms_or_thrust;
    }
    ...
    // log target
    //copter.Log_Write_Guided_Attitude_Target(drawstar_mode, roll_rad, pitch_rad, yaw_rad, ang_vel, drawstar_angle_state.thrust, drawstar_angle_state.climb_rate_cms * 0.01);
}
...
void ModeDrawStar::pos_control_run()
{
    ...
    if (drawstar_pos_terrain_alt) {
        pos_offset_z_buffer = MIN(copter.wp_nav->get_terrain_margin() * 100.0, 0.5 * fabsf(drawstar_pos_target_cm.z));
    }
    pos_control->input_pos_xyz(drawstar_pos_target_cm, terr_offset, pos_offset_z_buffer);
...
// called from guided_run
void ModeDrawStar::accel_control_run()
{
    // process pilot's yaw input
    float target_yaw_rate = 0;
    if (!copter.failsafe.radio && use_pilot_yaw()) {
    ...
    if (tnow - update_time_ms > get_timeout_ms()) {
        ...
        pos_control->input_vel_accel_xy(drawstar_vel_target_cms.xy(), drawstar_accel_target_cmss.xy(), false);
        pos_control->input_vel_accel_z(drawstar_vel_target_cms.z, drawstar_accel_target_cmss.z, false, false);
    } else {
        // update position controller with new target
        pos_control->input_accel_xy(drawstar_accel_target_cmss);
        ...
        pos_control->input_accel_z(drawstar_accel_target_cmss.z);
    }
...
// called from guided_run
void ModeDrawStar::velaccel_control_run()
{
    // process pilot's yaw input
    float target_yaw_rate = 0;
    if (!copter.failsafe.radio && use_pilot_yaw()) {
    ...
    copter.avoid.adjust_velocity(drawstar_vel_target_cms, pos_control->get_pos_xy_p().kP(), pos_control->get_max_accel_xy_cmss(), pos_control->get_pos_z_p().kP(), pos_control->get_max_accel_z_cmss(), G_Dt);
    ...
    if (!stabilizing_vel_xy() && !do_avoid) {
        ...
        drawstar_vel_target_cms.x = pos_control->get_vel_desired_cms().x;
        drawstar_vel_target_cms.y = pos_control->get_vel_desired_cms().y;
    }
    pos_control->input_vel_accel_xy(drawstar_vel_target_cms.xy(), drawstar_accel_target_cmss.xy(), false);
    if (!stabilizing_vel_xy() && !do_avoid) {
        // set position and velocity errors to zero
...
// called from guided_run
void ModeDrawStar::posvelaccel_control_run()
{
    // process pilot's yaw input
    float target_yaw_rate = 0;
    ...
    if (!stabilizing_vel_xy()) {
        ...
        drawstar_pos_target_cm.x = pos_control->get_pos_target_cm().x;
        drawstar_pos_target_cm.y = pos_control->get_pos_target_cm().y;
        drawstar_vel_target_cms.x = pos_control->get_vel_desired_cms().x;
        drawstar_vel_target_cms.y = pos_control->get_vel_desired_cms().y;
    } else if (!stabilizing_pos_xy()) {
    ...
    pos_control->input_pos_vel_accel_xy(drawstar_pos_target_cm.xy(), drawstar_vel_target_cms.xy(), drawstar_accel_target_cmss.xy(), false);
    if (!stabilizing_vel_xy()) {
        // set position and velocity errors to zero
        pos_control->stop_vel_xy_stabilisation();
    } else if (!stabilizing_pos_xy()) {
    ...
    float pz = drawstar_pos_target_cm.z;
    pos_control->input_pos_vel_accel_z(pz, drawstar_vel_target_cms.z, drawstar_accel_target_cmss.z, false);
    drawstar_pos_target_cm.z = pz;
...
void ModeDrawStar::angle_control_run()
{
    ...
    if (!drawstar_angle_state.use_thrust) {
        // constrain climb rate
        climb_rate_cms = constrain_float(drawstar_angle_state.climb_rate_cms, -wp_nav->get_default_speed_down(), wp_nav->get_default_speed_up());
    ...
    uint32_t tnow = millis();
    if (tnow - drawstar_angle_state.update_time_ms > get_timeout_ms()) {
        drawstar_angle_state.attitude_quat.initialise();
        drawstar_angle_state.ang_vel.zero();
        climb_rate_cms = 0.0f;
        if (drawstar_angle_state.use_thrust) {
            ...
            pos_control->init_z_controller();
            drawstar_angle_state.use_thrust = false;
        }
    }

    // interpret positive climb rate or thrust as triggering take-off
    const bool positive_thrust_or_climbrate = is_positive(drawstar_angle_state.use_thrust ? drawstar_angle_state.thrust : climb_rate_cms);
    ...
    if (copter.ap.land_complete && (drawstar_angle_state.climb_rate_cms > 0.0f)) {
    ...
    // call attitude controller
    if (drawstar_angle_state.attitude_quat.is_zero()) {
        attitude_control->input_rate_bf_roll_pitch_yaw(ToDeg(drawstar_angle_state.ang_vel.x) * 100.0f, ToDeg(drawstar_angle_state.ang_vel.y) * 100.0f, ToDeg(drawstar_angle_state.ang_vel.z) * 100.0f);
    } else {
        attitude_control->input_quaternion(drawstar_angle_state.attitude_quat, drawstar_angle_state.ang_vel);
    }

    // call position controller
    if (drawstar_angle_state.use_thrust) {
        attitude_control->set_throttle_out(drawstar_angle_state.thrust, true, copter.g.throttle_filt);
    } else {
...
{
    drawstar_limit.timeout_ms = 0;
    drawstar_limit.alt_min_cm = 0.0f;
    drawstar_limit.alt_max_cm = 0.0f;
    drawstar_limit.horiz_max_cm = 0.0f;
}
...
{
    drawstar_limit.timeout_ms = timeout_ms;
    drawstar_limit.alt_min_cm = alt_min_cm;
    drawstar_limit.alt_max_cm = alt_max_cm;
    drawstar_limit.horiz_max_cm = horiz_max_cm;
}
...
void ModeDrawStar::limit_init_time_and_pos()
{
    ...
    drawstar_limit.start_time = AP_HAL::millis();

    // initialise start position from current position
    drawstar_limit.start_pos = inertial_nav.get_position_neu_cm();
}
...
bool ModeDrawStar::limit_check()
{
    ...
    if ((drawstar_limit.timeout_ms > 0) && (millis() - drawstar_limit.start_time >= drawstar_limit.timeout_ms)) {
        return true;
    }
    ...
    if (!is_zero(drawstar_limit.alt_min_cm) && (curr_pos.z < drawstar_limit.alt_min_cm)) {
        return true;
    }

    // check if we have gone above max alt
    if (!is_zero(drawstar_limit.alt_max_cm) && (curr_pos.z > drawstar_limit.alt_max_cm)) {
        return true;
    }

    // check if we have gone beyond horizontal limit
    if (drawstar_limit.horiz_max_cm > 0.0f) {
        const float horiz_move = get_horizontal_distance_cm(drawstar_limit.start_pos.xy(), curr_pos.xy());
        if (horiz_move > drawstar_limit.horiz_max_cm) {
            return true;
        }
    }
...
uint32_t ModeDrawStar::wp_distance() const
{
    ...
    case SubMode::Pos:
        return get_horizontal_distance_cm(inertial_nav.get_position_xy_cm(), drawstar_pos_target_cm.tofloat().xy());
    case SubMode::PosVelAccel:
...
int32_t ModeDrawStar::wp_bearing() const
{
    ...
    case SubMode::Pos:
        return get_bearing_cd(inertial_nav.get_position_xy_cm(), drawstar_pos_target_cm.tofloat().xy());
    case SubMode::PosVelAccel:
